...
def save(course):
    sql = "INSERT INTO courses (title, level, date, capacity, active) VALUES (%s, %s, %s, %s, %s) RETURNING id"
    values = [course.title, course.level, course.date, course.capacity, course.active]
...
def select(id):
    ...
    course = Course(result["title"], result["level"], result["date"], result["capacity"], result["active"], result["id"])
    return course
